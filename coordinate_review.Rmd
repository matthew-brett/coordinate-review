...
* `src/skimage/draw/draw.py`: `rectangle`.  `start` input argument is:
  ...
  This does assume a semantic meaning to the first dimension, and to the
  second and third, which are "row" and "column".  One option is to change docstring to:
...
* Various `filter.rank` functions calling `_core_3D` in
  `filters/rank/core_cy_3d.pyx`.  Input image is 3D, and arguments `shift_x`,
  `shift_y`, and `shift_z`.  But these appear, oddly, to be offsets in `i, j,
  k` respectively of the `footprint`.   The first axis does appear to be
  special in some way.
