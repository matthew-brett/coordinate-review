...
* `predict_x`, `predict_y`, `predict_xy` in transform classes.  The transform
  ...
  to `predict(x, axis=0)[:, 1]`. We have `predict_xy` for `CircleModel` and `EllipseModel`.  We could rename those to `predict_coords`.
...
The following are in `transform/hough_transform.py`.

* Note multiple instances of `(M, N)`, replace with `(I, J)`.  Note output
  specification for `H` in `hough_circle`: edit for M, N etc.
