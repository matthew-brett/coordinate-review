...
* Various `filter.rank` functions calling `_core_3D` in
  ...
  axes of the footprint, or has the same number of elements as there are axes
  in `image`. If `shift` is a sequence, specify shift in `i, j[, k]` order.
  Note that, for 2D images, `shift_x` in Skimage1 corresponds to `j` (the
  second axis of the footprint) and `shift_y` corresponds to `i` (first axis
  of footprint). Thus a call with `shift_x=1, shift_y=2` for Skimage1 should
  become `shift=(2, 1)` in Skimage2, and a call with `shift_y=2` (and no
  `shift_x`) would be `shift=(2, 0)` in Skimage2.  For 3D images, in
  Skimage1, `shift_x`, `shift_y`, and `shift_z` correspond to `i, j, k` (the
  first, second and third axes of the footprint).  Thus a call with
