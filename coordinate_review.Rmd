...
* `denoise_wavelet` in `skimage/restoration/_denoise.py` has the following
  docstring:
...
* `denoise_nl_means` in `skimage/restoration/non_local_means.py`.  Docstring
  is:
  ...
  There are various uses of the term "plane" in `skimage/restoration/_nl_means_denoising.pyx`.  Plane seems to always mean "slice along first axis".
...
* `rag_mean_color` in `src/skimage/graph/_rag.py`.  Docstring:
...
* `show_rag` in `src/skimage/graph/_rag.py`.  Docstring:
  ...
  image : N-D ndarray
      ...
      `channel_axis=-1` (the default), then `image` should be shape (I, J, C),
...
* `autolevel_percentile`, `gradient_percentile`, `mean_percentile`, in
  `skimage/filters/rank/_percentile.py`.  Docstrings have form:
  ...
  image : ndarray of shape (I, J) and dtype (uint8 or uint16)
      Input image.
  footprint : ndarray of shape (A, B)
      The neighborhood expressed as a 2-D array of 1's and 0's.
...
* In `skimage/filters/lpi_filter.py`: several instances of `(M, N)`.  Can all
...
* `skimage/filters/edges.py`: all instances of `(M, N)` can be
  replaced with `(I, J)`.

* `skimage/segmentation/_chan_vese.py`: all instances of `(M, N)` can be
  replaced with `(I, J)`.

* `skimage/segmentation/thresholding.py`: all instances of `(M, N)` can be
  replaced with `(I, J)`.  All instances of `(M, N[, ...])` can be replaced by
  `(I, J[, ...])`.

* `skimage/io/_io.py`: `imsave`. Docstring:
...
* `src/skimage/segmentation/_quickshift.py`: `quickshift`:
  ...
  image : 3D ndarray
      ...
      via the `channel_axis` argument.
  ```
* `src/skimage/segmentation/_quickshift_cy.pyx` - replace `(M, N, C)` and `(M,
  N)` with `(I, J, C)` and `(I, J)` throughout.
* `src/skimage/filters/_sparse.py`: — replace `(M, N[, ...], P)` with `(I, J[,
  ...], K)`, `(M, N)` with `(I, J)`.
* `src/skimage/filters/morphsnakes.py`: — replace `(L, M, N)`, `(M, N)` with
  `(I, J, K)` and `(I, J)` throughout.
* `src/skimage/transform/hough_transform.py`: — replace `(M, N)` with and `(I,
  J)` throughout.  There is much else to do for coordinates in the
  `hough_transform` module — we need to rephrase outputs in terms of ij,
  instead of xy, including angles.  See below.
...
* `predict_x`, `predict_y`, `predict_xy` in transform classes.  The transform
  ...
  first coordinate column, and so on.

  We have various options.

  One is to deprecate `predict_x`, `predict_`, in favor of using `predict`.
  `predict_x` delegates to `predict(y, axis=1)[:, 0]`, `predict_y` delegates
  to `predict(x, axis=0)[:, 0]`. We have `predict_xy` for `CircleModel` and `EllipseModel`.  We could rename those to `predict_coords`.

  We could simply rename to reflect the fact that `x` and `y` correspond to the first and second coordinates columns, as in `predict_c0`, `predict_c1`, `predict_c01`.
...
* `skimage/feature/peak.py`: `_prominent_peaks`.  Docstring:
...
* `skimage/transform/_warps.py`: `swirl`
  ...
  ```rest
  There are is one key differences between this routine (in Skimage1) and the equivalent routine in Skimage2.

  The difference is that Skimage2 specifes the center for rotation as
  a coordinate in i, j (row, column) order, whereas Skimage1 asked for the
  center in (column, row) order.
...
* `skimage/transform/_warps.py`: `rotate`
  ...
  The first is that Skimage2 expects *radians* for the `angle` argume to rotate.  Skimage2 expected *degrees*.
...
Generally we give angles in radians.  There are size and a half exceptions:
...
* `transform/_geometric.py::euler_rotation_matrix` has a `degrees=False`
  keyword argument, allowing the user to use degrees instead of default
  radians.  A Github search of `/[ .]_euler_rotation_matrix\(/ AND NOT path:skimage` finds no-one using this with the `degrees=True` option.

Proposal: standardize to radians throughout.
...
* Hough transforms return lines in 

## Color and `channel_axis`

While exploring coordinates, I found several instances where the color channel is assumed to be the last.  Currently discussing whether these should now have `channel_axis=` in the Skimage2 argument list:

* `manual_segmentation` and `manual_lasso_segmentation` in
  `src/skimage/future/manual_segmentation.py` (`image : (M, N[, 3]) array`)
  Question on Zulip; should `future` modules be ported?
* `set_color` in `src/skimage/draw/draw.py` (`image : ndarray of shape (M, N,
  C)`)
* `rag_mean_color` and `show_rag` in `src/skimage/graph/_rag.py`.
