...
* Various `filter.rank` functions calling `_core_3D` in
  ...
  We should deprecate `shift_{x,y,z}`, in favor of `shift_{i,j,k}` and make
  keyword only.   The question is what to do for porting to Skimage 2, because the right port depends on the dimensionality of the image.

  Searches:

  ```/(autolevel|equalize|gradient|majority|maximum|mean|geometric_mean|subtract_mean|median|minimum|modal|enhance_contrast)\(.*?,.*?,.*shift_[xyz]\s*=/ AND NOT path:skimage AND (path:.py OR path:.ipynb)`
  ```

  and

  ```
  /(pop|threshold|noise_filter|entropy|otsu|sum)\(.*?,.*?,.*shift_[xyz]\s*=/ AND NOT path:skimage AND (path:.py OR path:.ipynb)
  ```

  Change
  `shift_x` and `shift_y` for 2D to `shift_j`, `shift_i` respectively, and
  make keyword only.  Rename `shift_`.
...
* `segmentation/_watershed.py` `watershed` function:
  ...
  This looks like it is an N-D function.

Other instances (via `ripgrep`) of `plane` to review in code-base:

```
tests/skimage/color/test_colorlabel.py:    # Labeling a 3D stack of duplicates gives the same result in each plane
tests/skimage/color/test_colorlabel.py:    for labeled_plane in labeled_3d:
tests/skimage/color/test_colorlabel.py:        assert_array_equal(labeled_plane, labeled_2d)
tests/skimage/restoration/test_rolling_ball.py:    # infinite plane light source at top left corner
src/skimage/measure/fit.py:        """Predict intersection of line model with orthogonal hyperplane.
src/skimage/measure/fit.py:            Axis orthogonal to the hyperplane intersecting the line.
