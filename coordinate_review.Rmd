...
experienced users of Numpy — it is the array coordinate system, also known as
the *matrix* or *linear algebra* coordinate system.  We usually call this the
"i, j" coordinate system, for reasons that should become clear.
...
*imaging coordinate system*, or the "x, y" coordinate system.
...
We will therefore call "row" and "column" and "plane" *semantic* axis labels,
because each tells about what that particular axis means in terms of image
display.
...
## Plans: use of plane in the codebase
...
See `watershed_3d_axes.ipynb` for an example.
...
* Various `filter.rank` functions calling `_core_3D` in
  ...
  We should deprecate `shift_{x,y,z}`, in favor of a generic `shifts` argument,
...
* `segmentation/slic_superpixels.py`: `slic` function.  Docstring:
  ...
  background — see `ai_outputs/slic_transpose_invariance.md`.  Indeed the notebook `slic_3d_axes.ipynb` confirms invariance for a version of the brain image without ties.  That's good enough for now — along with some confirmation from someone who knows the algorithm.  Awaiting.

  Provisional plan:

  We should rewrite the docstring to remove the apparent assumption that the
  channel axis is last, and the plane axis is first.  ... Rewrite Cython docstring to reflect transpose invariance for all axes but color.
