...
* Various `filter.rank` functions calling `_core_3D` in
  ...
  Input image can be 3D, and arguments `shift_x`, `shift_y`, and `shift_z`.
  But the `shift_` arguments appear, oddly, to be offsets in `i, j, k`
  respectively of the `footprint`.   The first axis does appear to be special
  in some way.
...
* `src/skimage/restoration/deconvolution.py` :
  ...
  This ends up calling `scipy.signal.convolve` on the input array, so no semantic meaning to the axes, and docstring better put as:
