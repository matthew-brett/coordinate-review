...
```{python}
# Reduce image, and make unique
rng = np.random.default_rng()

def proc_img(img):
    less_img = img[:20, ::2, ::2]
    noise = rng.normal(0, 0.01, size=less_img.shape)
    fimg = ski.util.img_as_float64(less_img) + noise
    assert len(np.unique(fimg)) == fimg.size
    return fimg
```

```{python}
imgs = [proc_img(img) for img in imgs]
```
...
```{python}
img = imgs[0]
ws_orig = ss.watershed(img, get_markers(img))
```

```{python}
def rolled_ws(img, axes):
    r_img = np.transpose(img, axes)
    f_r_img = ss.watershed(r_img, get_markers(r_img))
...
assert_labels_equivalent(ws_rolled, ws_orig)
```
...
for i, img in enumerate(imgs):
    ws_start = ss.watershed(img, get_markers(img))
    print(f'Image {i}')
    for order in orderings:
        print(f'Ordering {order}')
        ws_s_r = rolled_ws(img, order)
        assert_labels_equivalent(ws_s_r, ws_start)
```
